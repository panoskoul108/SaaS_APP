...
export default function Menu() {
  ...
  const confirmAddons = () => {
    ...
    (activeProduct.addons || []).forEach((g) => {
      ...
      if (sels.length > 0) {
        const names = sels.map((idx) => g.options[idx].name); // Αποθηκεύουμε ΠΑΝΤΑ το ελληνικό στη βάση
    ...
    const baseName = activeProduct.name; // Αποθηκεύουμε ΠΑΝΤΑ το ελληνικό στη βάση
    const finalName =
      addonTexts.length > 0
      ...
      .insert([
        {
          ...
          payment_method: paymentMethod, // Θα είναι "ΜΕΤΡΗΤΑ" ή "ΚΑΡΤΑ" γιατί το ελέγχουμε πιο κάτω
          status: "pending",
  ...
  // --- Η ΝΕΑ ΣΥΝΑΡΤΗΣΗ ΓΙΑ ΔΥΝΑΜΙΚΗ ΜΕΤΑΦΡΑΣΗ ΣΤΟ ΚΑΛΑΘΙ / ΙΣΤΟΡΙΚΟ ---
  const getItemDisplayName = (item) => {
    const orig = products.find((p) => p.id === item.id);
    if (!orig) return item.name; // Αν έχει διαγραφεί το προϊόν, δείξε το παλιό
  ...
  // --- ΕΔΩ ΠΕΡΝΑΜΕ ΤΗ ΓΛΩΣΣΑ ΚΑΙ ΤΑ ΠΡΟΪΟΝΤΑ ΣΤΟ ORDER STATUS ---
                ...
                <div className="flex justify-between items-start">
                  <div className="flex-1 pr-3">
                    <h4 className="font-black uppercase text-gray-800 text-sm leading-tight">
                      {getItemDisplayName(item)} {/* ΔΥΝΑΜΙΚΗ ΜΕΤΑΦΡΑΣΗ ΕΔΩ */}
                    </h4>
                    {item.note && (
